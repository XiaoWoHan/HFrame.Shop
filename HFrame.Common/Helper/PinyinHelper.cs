...
namespace HFrame.Common.Helper
{
    ...
    public class PinyinHelper
    {
        ...
        /// 包含字符 ASC 码的整形数组
        /// </SUMMARY>
        private static readonly int[] _PINYINV = new[]
                                                         {
        ...
        /// 包含汉字拼音的字符串数组。
        /// </SUMMARY>
        private static readonly string[] _PINYINN = new[]
                                                           {
        ...
        public static string CHSToPinyin(string chs, string separator="", bool initialCap=false)
        {
            ...
            {
                byte[] array = Encoding.Default.GetBytes(item.ToString());
                string ItemPinyin = item.ToString();
                if (ValidateHelper.IsChinese(ItemPinyin))
                {
                    int i1 = (array[0]);
                    int i2 = (array[1]);
                    int chrasc = i1 * 256 + i2 - 65536;//计算当前字ASC值
                    var Key = _PINYINV.Where(m => m <= chrasc).Select((m, key) => key).Max();//下标
                    var Name = _PINYINN[Key];//当前拼音
                    if (initialCap) Name = Thread.CurrentThread.CurrentCulture.TextInfo.ToTitleCase(Name);//转换首字母大写
                    ItemPinyin = Name;
                }
                Results.Add(ItemPinyin);
            }
